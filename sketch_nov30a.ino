...
int Pinx = analogRead(A0);
int Piny = analogRead(A1);

#define D 1
#define G 1 //センサ値読み取り回数
...
float Ztheta = 0.00;
float aX, aY, aZ, aSqrt, gX, gY, gZ, mDirection, mX, mY, mZ, X, Y, Z;
...
//  aY = mySensor.accelY();
  ...
  //Serial.print(" Yaheni: " + String(Yaheni));
  //Serial.print(" Yamove: " + String(Yamove));
  //Serial.print(" Ygheni: " + String(Ygheni));
  //Serial.print(" Ygmove: " + String(Ygmove));


  
  Ztheta = acos( gZ / sqrt( gX*gX + gY*gY + gZ*gZ ) );
  Serial.print(" Ztheta: " + String(Ztheta));


  double Yaval,sum = 0;
  aY = 0;
  for(int i=0;i<G-1;i++){
    YaArray[i] = YaArray[i+1];
     aY += YaArray[i];
  }
  YaArray[G-1] = mySensor.accelY();
  aY += YaArray[G-1];
  aY /= G;
  ...
  Serial.print(" Yaval: " + String(Yaval) );

  
  Yaheni = Yaval-Yamemo;
  Yamove = Yaheni*180*400*16/360; //半周分xステップ数x16倍x一周で割る
  ...
  if(analogRead(A1) > 500){
    L6470_move(0,int(3*400*16/360*(analogRead(A1)-487)/536));
  }else if(analogRead(A1) < 470){
    L6470_move(1,int(3*400*16/360*(-analogRead(A1)+487)/487));
  }//3xはゲイン
/*
...
L6470_setparam_stepmood(0x05); //ステップモードdefault 0x07 (1+3+1+3bit)
}
